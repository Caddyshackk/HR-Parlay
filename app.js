...
class HRParlayApp {
    ...
    renderGame(game) {
        ...
        if (awayPitcher && homePitcher) {
            ...
            // Determine pitcher quality
            const awayQuality = awayPitcherHR9 >= 1.5 ? '🎯 Hittable' : awayPitcherHR9 <= 1.0 ? '❄️ Tough' : '⚖️ Average';
            const homeQuality = homePitcherHR9 >= 1.5 ? '🎯 Hittable' : homePitcherHR9 <= 1.0 ? '❄️ Tough' : '⚖️ Average';
            
            const awayQualityClass = awayPitcherHR9 >= 1.5 ? 'hittable' : awayPitcherHR9 <= 1.0 ? 'tough' : 'average';
            const homeQualityClass = homePitcherHR9 >= 1.5 ? 'hittable' : homePitcherHR9 <= 1.0 ? 'tough' : 'average';
            
            pitcherMatchupHTML = `
                <div class="pitcher-matchup-header">
                    <div class="matchup-title">⚔️ Pitching Matchup</div>
                    <div class="pitcher-row ${awayQualityClass}">
                        <div class="pitcher-name-header">
                            <strong>${awayPitcher.fullName}</strong> (${awayHand}HP)
                        </div>
                        <div class="pitcher-stats-inline">
                            ERA: ${awayPitcherERA.toFixed(2)} | HR/9: ${awayPitcherHR9.toFixed(2)} | ${awayQuality}
                        </div>
                    </div>
                    <div class="pitcher-row ${homeQualityClass}">
                        <div class="pitcher-name-header">
                            <strong>${homePitcher.fullName}</strong> (${homeHand}HP)
                        </div>
                        <div class="pitcher-stats-inline">
                            ERA: ${homePitcherERA.toFixed(2)} | HR/9: ${homePitcherHR9.toFixed(2)} | ${homeQuality}
                        </div>
                    </div>
                </div>
            `;
