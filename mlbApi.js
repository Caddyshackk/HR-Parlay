...
class MLBApi {
            ...
            try {
                ...
                const hitters = splits
                    ...
                    .map(s => {
                        ...
                        // Project HR pace over a full 550 AB season
                        // If player has HRs: use current rate
                        ...
                        return {
                            ...
                            seasonHRs: projectedHRs,
                            displayHRs: seasonHRs,
                            ...
                            earlySeasonProjected: atBats < 80,
                        };
                    });

                if (hitters.length > 0) {
                    // For players with 0 current HRs, fetch prior year to set power baseline
                    await this.enrichWithPriorYear(hitters, season - 1);
                    // Fetch last-7-days game logs
                    await this.enrichLast7HRs(hitters, season);
    ...
    // Fetch last 7 days of game logs for each hitter and sum HRs
    ...
    async enrichWithPriorYear(hitters, priorSeason) {
        ...
        await Promise.allSettled(needsBaseline.map(async hitter => {
            try {
                const cacheKey = `prior_${hitter.id}_${priorSeason}`;
                if (this._cache[cacheKey]) {
                    hitter.seasonHRs = Math.max(hitter.seasonHRs, this._cache[cacheKey].data);
                    return;
                }

                const url = `${this.baseUrl}/people/${hitter.id}/stats` +
                    `?stats=season&group=hitting&season=${priorSeason}`;
                const res = await fetch(url);
                if (!res.ok) return;

                const data = await res.json();
                const splits = data?.stats?.[0]?.splits || [];
                const priorHRs = splits[0]?.stat?.homeRuns || 0;
    ...
    async getEnhancedPitcherStats(pitcherId) {
        ...
        try {
            ...
            const [seasonRes, logRes] = await Promise.all([
                ...
                fetch(`${this.baseUrl}/people/${pitcherId}/stats?stats=gameLog&group=pitching&season=2026&limit=5`)
            ]);
